 ...
 */
class CheckQrRefundsShell extends DashBoardShell
{
    use ModelAwareTrait;
    ...
    public function main()
    {
        ...
        try {
            ...
            $iNgCodeTrades = $this->TINgCodeTrade->find('all',[
                'conditions'=> [
                    'Update_Date <' => date('Y/m/d H:i:s', strtotime('-'.$this->config['QRPAY']['CheckQrRefunds']['WaitingTime'].' second')),
                    'Status_ID' => 1,
                ],
            ]);
            ...
            foreach($iNgCodeTrades as $iNgCodeTrade){
                ...
                try{
                    $ret = $this->httpPostRequest(
                        $this->baseUrl . Router::url(['controller' => 'Qrpays', 'action' => 'checkrefunds']),
                        ['Content-Type:application/json'],
                        [
                            'storeRefundId'=> $iNgCodeTrade['Store_Refund_ID'],
                    ...
                    if($ret['retCd'] == 0 && $ret['resData'] && is_array($ret['resData'])){
                        if($ret['resData']['http_status'] != 200){
                            // エラー  --> 5分後に再チェック
                            throw new \Exception(' qrpays.checrefunds error (http_status='.$ret['resData']['http_status'].')', 810, $ret['exception']);
                        }
                        if($ret['resData']['returnCode'] == 'MP10000' && $ret['resData']['result'] && is_array($ret['resData']['result'])){
                            switch($ret['resData']['result']['result_code']){
                                case 'SUCCESS':     // 決済完了 -->  3(finish)にして処理終了
                                case 'FINISHED':    // 決済完了 -->  3(finish)にして処理終了
                                    $iNgCodeTrade['Status_ID'] = 3;
                                    $iNgCodeTrade['Update_User'] = $this->config['QRPAY']['CheckQrRefunds']['UpdateUserName'];
                        ...
                        // エラー  --> 5分後に再チェック
                        throw new \Exception(' qrpays.checkrefunds error (retCd='.$ret['retCd'].')', 820, $ret['exception']);
                    }
                } catch (\Exception $e) {
                    $cntError++;
